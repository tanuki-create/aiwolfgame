...
export class GameFSM {
  ...
  private getWarningMessage(remainingSeconds: number): string {
    ...
    const phaseName = phaseNameMap[this.state.phase] || 'フェーズ';

    if (remainingSeconds === 60) {
      return `⏰ ${phaseName}残り1分です`;
    } else if (remainingSeconds === 30) {
      return `⚠️ ${phaseName}残り30秒です`;
    } else if (remainingSeconds === 10) {
      return `🔔 ${phaseName}残り10秒です！`;
    }
    
    return `⏰ ${phaseName}残り${remainingSeconds}秒です`;
  }
